...
export enum LessonType {
  REGULAR = 'REGULAR',              // one class, one teacher, one room
  SHARED = 'SHARED',                // two classes, same room, one teacher
  PARALLEL = 'PARALLEL',            // two classes, each gets its own teacher + room (one lesson record, two LessonTeacher entries)
  MATH_GROUP = 'MATH_GROUP',        // cross-class math level group (3/4/5 pt bagrut)
  ENGLISH_GROUP = 'ENGLISH_GROUP',  // cross-class English level group (same structure as MATH_GROUP)
  MULTI_TEACHER = 'MULTI_TEACHER',  // two classes, one room, multiple teachers (one lesson record, multiple LessonTeacher entries)
}
...
export enum RestrictionTier {
  /**
   * Hard physical impossibilities — teacher in two places at once, class
   * double-booked, same subject twice on same day.  Generated by code, never
   * stored in the DB, cannot be overridden by admins.
   */
  INVARIANT = 'INVARIANT',
...
export const TIER_LABEL: Record<RestrictionTier, string> = {
  [RestrictionTier.INVARIANT]: 'Invariant',
 ...
 *
 * INVARIANT is 1 billion — one violation dominates any realistic count of
 * soft-constraint violations (NON_NEGOTIABLE × 200 = 20M < 1B).
 */
export const TIER_PENALTY: Record<RestrictionTier, number> = {
  [RestrictionTier.INVARIANT]: 1_000_000_000,
