...
// All tiers are user-selectable, including INVARIANT.
// INVARIANT means "the auto-scheduler treats this as physically impossible":
// the penalty is 1 billion and SA never accepts a violation. It does NOT cause
// the AS to fail outright (Gate 2 only blocks on hard D-category invariants).
// Use it for contractual/physical constraints that must never be violated.
const USER_TIERS = Object.values(RestrictionTier)

// Tier visual indicators
const TIER_DOT: Record<RestrictionTier, string> = {
  [RestrictionTier.INVARIANT]:      '#7c3aed',   // purple — matches TeacherAvailabilityModal
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        {tier === RestrictionTier.INVARIANT && (
          <p className="mt-1.5 text-[11px] px-1" style={{ color: '#7c3aed' }}>
            ⛔ Invariant — the auto-scheduler treats this as physically impossible and will
            never place a lesson that violates it (1B penalty, never accepted by SA).
          </p>
        )}
      </div>
